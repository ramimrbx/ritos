...
def main():
    ...
    all_pixels = []
    for idx, (svg_name, var, label) in enumerate(ICONS):
        svg_path = f"assets/icons_raw/{svg_name}.svg"
        if os.path.exists(svg_path):
            ...
            fc = FALLBACK_COLORS[idx % len(FALLBACK_COLORS)]
            pixels = _make_fallback(SIZE, fc, label[0])
        all_pixels.append((var, label, pixels))

    with open('assets/icons/icons_32.h', 'w') as f:
        f.write('#ifndef ICONS_32_H\n#define ICONS_32_H\n#include <stdint.h>\n\n')
        f.write(f'#define ICON_SIZE 32\n#define ICON_COUNT {len(ICONS)}\n\n')
        for (var, label, pixels) in all_pixels:
        ...
        for (var, _, _) in all_pixels:
            f.write(f'  ICON_{var},\n')
        f.write('};\n\n')

        f.write(f'static const char* const g_icon_labels[ICON_COUNT] = {{\n')
        for (_, label, _) in all_pixels:
            f.write(f'  "{label}",\n')
        f.write('};\n\n#endif\n')
    print("Generated assets/icons/icons_32.h")
