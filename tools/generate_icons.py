...
ICONS = [
    ("monitor",   "SYSMON",    "SysMon",    "system_monitor"),
    ("calculator","CALC",      "Calc",      "calculator"),
    ("editor",    "EDITOR",    "Editor",    "text_editor"),
    ("files",     "FILES",     "Files",     "file_manager"),
    ("clock",     "CLOCK",     "Clock",     "clock"),
    ("calendar",  "CALENDAR",  "Calendar",  "calendar"),
    ("settings",  "SETTINGS",  "Settings",  "settings"),
    ("terminal",  "TERMINAL",  "Terminal",  "terminal"),
    ("imgview",   "IMGVIEW",   "ImgViewer", "image_viewer"),
]
...
def main():
    ...
    for idx, (svg_name, var, label, app) in enumerate(ICONS):
        svg_path = f"assets/sources/icons/{svg_name}.svg"
    ...
    with open('assets/generated/icons_32.h', 'w') as f:
        ...
        for (_, label, _, _app) in all_pixels:
            f.write(f'  "{label}",\n')
        f.write('};\n\n#endif\n')
    print("Generated assets/generated/icons_32.h")

    # Raw ARGB blobs, one per app, embedded into each .rbx by elf2rbx
    # (--icon=...) so executables carry their own icon.
    import struct
    os.makedirs('assets/generated/icons', exist_ok=True)
    for (_, _, pixels, app) in all_pixels:
        with open(f'assets/generated/icons/{app}.argb', 'wb') as f:
            f.write(struct.pack(f'<{len(pixels)}I', *pixels))
        print(f"Generated assets/generated/icons/{app}.argb")
