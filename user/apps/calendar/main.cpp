...
extern "C" rbx_module* rbx_module_init(const RitOS_API* api, const Desktop_Interface* desktop) {
	...
	static ritos::CalendarWindow* cal = nullptr;
	if (!cal) {
		cal = new ritos::CalendarWindow(48, 12);
	}

	static rbx_module mod;
	mod.type = RBX_MODULE_APP_WINDOW;
	const char* nm = "Calendar";
	int i = 0;
	while (nm[i] && i < 31) { mod.name[i] = nm[i]; i++; }
	mod.name[i] = '\0';
	mod.instance = cal;

	mod.draw = [](void* inst) { static_cast<ritos::CalendarWindow*>(inst)->draw(); };
	mod.handle_click = [](void* inst, int mx, int my) { static_cast<ritos::CalendarWindow*>(inst)->handle_click(mx, my); };
	mod.handle_key = [](void* inst, char key) { static_cast<ritos::CalendarWindow*>(inst)->handle_key(key); };
	mod.destroy = [](void* inst) { delete static_cast<ritos::CalendarWindow*>(inst); };
