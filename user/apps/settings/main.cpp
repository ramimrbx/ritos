...
extern "C" rbx_module* rbx_module_init(const RitOS_API* api, const Desktop_Interface* desktop) {
	...
	static ritos::SettingsWindow* set = nullptr;
	if (!set) {
		set = new ritos::SettingsWindow(30, 8, nullptr);
	}
	...
	mod.draw = [](void* inst) { static_cast<ritos::SettingsWindow*>(inst)->draw(); };
	mod.handle_click = [](void* inst, int mx, int my) { static_cast<ritos::SettingsWindow*>(inst)->handle_click(mx, my); };
	mod.handle_key = [](void* inst, char key) { static_cast<ritos::SettingsWindow*>(inst)->handle_key(key); };
	mod.destroy = [](void* inst) { delete static_cast<ritos::SettingsWindow*>(inst); };
