#include <ritos/apps.hpp>
#include <ritos/desktop.hpp>
#include <rit/vfs.hpp>
...
static bool path_has_prefix(const char* f, const char* pre) {
	int i = 0;
	while (pre[i]) { if (f[i] != pre[i]) return false; i++; }
	return true;
}

void FileExplorerWindow::refresh_files() {
	const char* tmp[64]; int total = rit::VFS::get_file_list(tmp, 64);
	m_file_count = 0;
	for (int i = 0; i < total && m_file_count < 64; i++) {
		const char* f = tmp[i];
		bool is_sys      = path_has_prefix(f, "/sys/");
		bool is_notes    = path_has_prefix(f, "/notes/");
		bool is_desktop  = path_has_prefix(f, "/desktop/");
		bool is_launcher = path_has_prefix(f, "/launcher/");
		...
		else if (m_left_selected_idx == 3 && is_desktop)  m_file_list[m_file_count++] = f;
		else if (m_left_selected_idx == 4 && is_launcher) m_file_list[m_file_count++] = f;
	}
}
...
void FileExplorerWindow::draw() {
	...
	const char* addr = (m_left_selected_idx==1) ? " Path: /sys"
	                 ...
	                 : (m_left_selected_idx==3) ? " Path: /desktop"
	                 : (m_left_selected_idx==4) ? " Path: /launcher"
	                                             : " Path: /";
	...
	struct { const char* label; rit::Color icon_fg; } folders[5] = {
		{" /       ", rit::Color::LightCyan},
		{" \x0E sys   ", rit::Color::LightGreen},
		{" \x0E notes ", rit::Color::LightGreen},
		{" \x0E desktp", rit::Color::LightGreen},
		{" \x0E launch", rit::Color::LightGreen}
	};
	for (int r = 0; r < 5; r++) {
	...
	for (int i=0; i<7; i++) {
		...
		if (i < m_file_count) {
			...
			if      (m_left_selected_idx==1 && path_has_prefix(dn, "/sys/"))      dn+=5;
			else if (m_left_selected_idx==2 && path_has_prefix(dn, "/notes/"))    dn+=7;
			else if (m_left_selected_idx==3 && path_has_prefix(dn, "/desktop/"))  dn+=9;
			else if (m_left_selected_idx==4 && path_has_prefix(dn, "/launcher/")) dn+=10;
			...
			int fsz=0;
			rit::VFS::read_file(m_file_list[i], &fsz);
			...
			const char* fn2 = m_file_list[i];
			int fnl=0; while (fn2[fnl]) fnl++;
			const char* typ = "TXT";
			if      (fnl>4 && fn2[fnl-4]=='.'&&fn2[fnl-3]=='r'&&fn2[fnl-2]=='b'&&fn2[fnl-1]=='x') typ="RBX";
			else if (fnl>5 && fn2[fnl-5]=='.'&&fn2[fnl-4]=='s'&&fn2[fnl-3]=='t'&&fn2[fnl-2]=='c'&&fn2[fnl-1]=='t') typ="STC";
			draw_str(typ, pfg, pbg, rx+21, ly);
		} else {
...
void FileExplorerWindow::handle_click(int mx, int my) {
	...
	if (ry>=7&&ry<=13) {
		if (rx>=2&&rx<=12) {
			int cf=ry-8;
			if (cf>=0&&cf<=4) { m_left_selected_idx=cf; m_active_pane=0; m_selected_idx=0; refresh_files(); }
		} else if (rx>=13&&rx<=35) {
			int ci=ry-7;
		...
		if (rx>=2&&rx<=7) {
			if (m_selected_idx>=0&&m_selected_idx<m_file_count) {
				const char* fp = m_file_list[m_selected_idx];
				int fl = 0; while (fp[fl]) fl++;
				// Opening an .rbx executes it, a .stct launches its target;
				// anything else opens in the editor
				if (fl>4 && fp[fl-4]=='.'&&fp[fl-3]=='r'&&fp[fl-2]=='b'&&fp[fl-1]=='x') {
					rit::System::launch_app(fp);
				}
				else if (fl>5 && fp[fl-5]=='.'&&fp[fl-4]=='s'&&fp[fl-3]=='t'&&fp[fl-2]=='c'&&fp[fl-1]=='t') {
					const char* t = rit::VFS::read_file(fp);
					if (t) rit::System::launch_app(t);
				}
				else if (m_editor) { m_editor->open_file(fp); m_editor->set_visible(true); m_editor->set_minimized(false); m_editor->set_active(true); }
				else { rit::VFS::create_file("/sys/open_file.txt",fp); rit::System::launch_app("Text Editor"); }
			}
		} else if (rx>=10&&rx<=14) {
...
void FileExplorerWindow::handle_key(char key) {
	...
	if (m_active_pane==0) {
		...
		else if (key=='s'||key==0x1F) { if (m_left_selected_idx<4) { m_left_selected_idx++; m_selected_idx=0; refresh_files(); } }
