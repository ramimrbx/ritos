 ...
 * The desktop is not hardcoded: every icon is a /desktop/<Name>.stct
 * shortcut file whose content is the target .rbx path. The label is the
 * shortcut filename (extension hidden); the pixels come from the icon
 * embedded in the target executable itself. */
...
static int slot_px(int idx) { return 20 + (idx / 4) * 60; }
static int slot_py(int idx) { return 40 + (idx % 4) * 80; }
...
static void load_shortcuts() {
    ...
    for (int i = 0; i < n && g_shortcut_count < MAX_SHORTCUTS; i++) {
        ...
        int li = 0;
        for (int k = p; k < fl - 5 && li < 23; k++) sc.label[li++] = f[k];
        sc.label[li] = '\0';
...
class Desktop {
    ...
    void draw_icon_slot(int idx) {
        ...
        int px = slot_px(idx), py = slot_py(idx);
        if (sc.icon) {
            g_api->fb_blit_argb(sc.icon, px, py, 32, 32);
        } else {
            /* Target has no embedded icon: generic placeholder */
            g_api->fb_fill_rounded_rect(px, py, 32, 32, 6, 0xFF5B8FFFu);
            char ch[2] = { sc.label[0], '\0' };
            g_api->fb_draw_string_px(ch, 0xFFFFFFFFu, 0, px + 12, py + 8, 1);
        }
        /* Label centered below icon */
        const char* lbl = sc.label;
        int llen = 0; while (lbl[llen]) llen++;
        int lx = px + (32 - llen * 8) / 2;
        g_api->fb_draw_string_px(lbl, C_LABEL, 0, lx, py + 34, 1);
    }
...
public:
    ...
    void handle_mouse_down(int px_x, int px_y) {
        ...
        if (col >= 0) {
            ...
            if (row >= 0) {
                int sc_idx = col * 4 + row;
                if (sc_idx < g_shortcut_count)
                    load_and_run_app(g_shortcuts[sc_idx].target);
            }
        }
    }
