...
public:
    ...
    void draw() override {
        ...
        // Navigation bar on the first row below the 32px titlebar
        int bar_y = m_y + 2;
        int bar_x = m_x + 1;
        // Clear nav bar
    ...
    void handle_click(int mx, int my) override {
        /* mx/my are pixels; the nav bar is the 16px row below the titlebar */
        int bar_py = m_y * 16 + 32;
        if (my >= bar_py && my < bar_py + 16) {
            if (mx < m_x * 8 + 72) {
                m_image_index = (m_image_index + 2) % 3;  // prev
            } else if (mx > m_x * 8 + m_width * 8 - 72) {
