...
private:
	...
	void execute_command(const char* cmd) {
		...
		if (str_equals(clean_cmd, "help")) {
			...
			add_line("  run <file.rbx> - run an executable");
			add_line("  disk - show physical disk info");
			add_line("  mount - import disk files to /volumes");
			add_line("  save <file> - write file to disk");
			...
			if (*args == '\0') {
				...
				int fp_len = 0;
				if (args[0] != '/') {
					const char* sys_path = "/users/ramim/documents/";
		...
		} else if (str_equals(clean_cmd, "run")) {
			if (*args == '\0') {
				add_line("Usage: run <file.rbx>");
			} else {
				char full_path[64];
				int fp_len = 0;
				if (args[0] != '/') {
					const char* sys_path = "/system/executables/";
					while (sys_path[fp_len]) { full_path[fp_len] = sys_path[fp_len]; fp_len++; }
				}
				int al = 0;
				while (args[al] && fp_len < 59) { full_path[fp_len++] = args[al++]; }
				bool has_ext = fp_len >= 4 &&
					full_path[fp_len-4]=='.' && full_path[fp_len-3]=='r' &&
					full_path[fp_len-2]=='b' && full_path[fp_len-1]=='x';
				if (!has_ext) {
					full_path[fp_len++]='.'; full_path[fp_len++]='r';
					full_path[fp_len++]='b'; full_path[fp_len++]='x';
				}
				full_path[fp_len] = '\0';

				if (g_api->vfs_exists(full_path)) {
					add_line(full_path);
					m_desktop->launch_app(full_path);
				} else {
					add_line("No such executable.");
				}
			}
		} else if (str_equals(clean_cmd, "disk")) {
			char info[256];
			g_api->storage_describe(info, 256);
			char line[80]; int ll = 0;
			for (int i = 0; ; i++) {
				if (info[i] == '\n' || info[i] == '\0' || ll >= 78) {
					line[ll] = '\0';
					if (ll > 0) add_line(line);
					ll = 0;
					if (info[i] == '\0') break;
				} else {
					line[ll++] = info[i];
		...
		} else if (str_equals(clean_cmd, "mount")) {
			int imported = g_api->storage_import();
			if (imported < 0) {
				add_line("No operable volume. Try: disk");
			} else {
				char buffer[40]; int bl = 0;
				const char* head = "Imported ";
				while (head[bl]) { buffer[bl] = head[bl]; bl++; }
				if (imported == 0) buffer[bl++] = '0';
				else {
					char digits[8]; int dn = 0; int v = imported;
					while (v) { digits[dn++] = (char)('0' + v % 10); v /= 10; }
					while (dn) buffer[bl++] = digits[--dn];
				}
				buffer[bl] = '\0';
				add_line(buffer);
				add_line("files into /volumes/disk");
			}
		} else if (str_equals(clean_cmd, "save")) {
			if (*args == '\0') {
				add_line("Usage: save <file>");
			} else {
				char full_path[64];
				int fp_len = 0;
				if (args[0] != '/') {
				...
				if (!g_api->vfs_exists(full_path)) {
					add_line("File not found.");
				} else if (g_api->storage_export(full_path)) {
					add_line("Saved to physical disk.");
				} else {
					add_line("Disk write failed.");
				}
			}
			...
			if (g_api->vfs_exists("/users/ramim/configuration/appearance.txt")) {
				int cfg_len = 0;
				const char* old_cfg = g_api->vfs_read_file("/users/ramim/configuration/appearance.txt", &cfg_len);
			...
			if (!wp_valid) {
				...
				g_api->vfs_write_file("/users/ramim/configuration/appearance.txt", new_cfg, 2);
...
public:
	...
	void draw() override {
		...
		if (!rit::System::fb_is_avail()) {
			...
			int base_y = m_y + 1;
			int row = 0;
			int start_slot = m_history_count > m_height - 3 ? m_history_count - (m_height - 3) : 0;
			for (int s = start_slot; s < m_history_count; s++) {
				for (int col = 0; m_history[s][col] && col < m_width - 4; col++)
					rit::System::draw_char(m_history[s][col], rit::Color::LightGreen, rit::Color::Black, base_x + col, base_y + row);
				row++;
			}
			rit::System::draw_char('>', rit::Color::LightGreen, rit::Color::Black, base_x, base_y + row);
			for (int i = 0; i < m_input_len; i++)
				rit::System::draw_char(m_input_buf[i], rit::Color::White, rit::Color::Black, base_x + 2 + i, base_y + row);
			return;
		}

		/* Windows-Terminal-style dark body */
		int wx = px(), wy = content_y(), ww = pw(), wh = content_h();
		rit::System::fb_fill_rect(wx + 1, wy, ww - 2, wh - 1, 0xFF0C0C0Cu);

		int line_h = 18;
		int tx = wx + 14, ty = wy + 10;
		int max_rows = (wh - 20) / line_h - 1;

		int start_slot = m_history_count > max_rows ? m_history_count - max_rows : 0;
		int row = 0;
		for (int s = start_slot; s < m_history_count; s++) {
			rit::System::fb_draw_string_px(m_history[s], 0xFFCCCCCCu, 0, tx, ty + row * line_h, 1);
			row++;
		}

		/* prompt line */
		int iy = ty + row * line_h;
		rit::System::fb_draw_string_px("> ", 0xFF16C60Cu, 0, tx, iy, 1);
		rit::System::fb_draw_string_px(m_input_buf, 0xFFF2F2F2u, 0, tx + 16, iy, 1);
		if (m_active)
			rit::System::fb_fill_rect(tx + 16 + m_input_len * 8, iy, 8, 16, 0xFFB0B0B0u);
	}
...
extern "C" rbx_module* rbx_module_init(const RitOS_API* api, const Desktop_Interface* desktop) {
	...
	if (!term) {
		term = new ritos::TerminalWindow(60, 12, 90, 30, desktop);
	}
	...
	mod.destroy = [](void* inst) { delete static_cast<ritos::TerminalWindow*>(inst); };
