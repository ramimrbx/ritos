...
extern "C" rbx_module* rbx_module_init(const RitOS_API* api, const Desktop_Interface* desktop) {
	...
	if (!editor) {
		editor = new ritos::TextEditorWindow(40, 8, 80, 36);

		// Check for parameter file from File Explorer
		if (api->vfs_exists("/temporary/open_file.txt")) {
			int len = 0;
			const char* filename = api->vfs_read_file("/temporary/open_file.txt", &len);
			if (filename && len > 0) {
				char temp_fn[32];
				int i = 0;
				while (i < len && i < 31) {
				...
				api->vfs_delete_file("/temporary/open_file.txt");
			}
		}
	}
	...
	mod.destroy = [](void* inst) { delete static_cast<ritos::TextEditorWindow*>(inst); };
