...
struct rbx_module {
	...
	// Callbacks. handle_click receives exact PIXEL coordinates.
...
struct Desktop_Interface {
	...
	/* Pixel-precise mouse position */
	int (*get_mouse_px_x)(void);
	int (*get_mouse_px_y)(void);

	/* 32x32 ARGB icon embedded in the window's executable (may be null) */
	const unsigned int* (*get_window_icon)(int index);
};
