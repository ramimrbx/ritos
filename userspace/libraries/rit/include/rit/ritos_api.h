...
struct RitOS_API {
	...
	// ── Real storage (physical disk + on-disk filesystem) ──────────────────
	/* Describe the detected disk and filesystem; 1 if operable FAT volume */
	int (*storage_describe)(char* out, int out_max);
	/* Import the volume's root directory into /volumes/Disk; returns count */
	int (*storage_import)(void);
	/* Write a VFS file out to the physical disk root; 1 on success */
	int (*storage_export)(const char* vfs_path);

	// ── Modern UI additions ─────────────────────────────────────────────────
	/* Nearest-neighbour scaled blit (alpha-blended) */
	void (*fb_blit_argb_scaled)(const uint32_t* src, int sw, int sh,
	                            int dx, int dy, int dw, int dh);
	/* Integer-scaled text, transparent background (for headings) */
	void (*fb_draw_string_scaled)(const char* text, uint32_t fg,
