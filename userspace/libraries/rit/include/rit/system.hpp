...
public:
	...
	static void fb_blit_argb_scaled(const uint32_t* src, int sw, int sh,
	                                int dx, int dy, int dw, int dh);
	static void fb_draw_string_scaled(const char* text, uint32_t fg,
	                                  int x, int y, int scale);
	static int  fb_draw_wallpaper();
