...
void VFS::init() {
	...
	create_file("/system/manifest.txt",
		"RitOS\n"
		"architecture: x86\n"
		"executable format: RBX version 2\n"
	);
	create_file("/users/ramim/documents/readme.txt", 
	...
	create_file("/users/ramim/documents/todo.txt", 
...
bool VFS::create_file(const char* name, const char* content, int length) {
	...
	for (int i = 0; i < VFS_MAX_FILES; i++) {
		if (!g_files[i].is_used) {
			str_copy(g_files[i].name, name, 64);
			if (content) {
				if (length < 0) {
					length = 0;
...
bool VFS::write_file(const char* name, const char* content, int length) {
	...
	/* Writing a file that doesn't exist yet creates it */
	return create_file(name, content, length);
}
...
bool VFS::rename_file(const char* old_name, const char* new_name) {
	...
	for (int i = 0; i < VFS_MAX_FILES; i++) {
		if (g_files[i].is_used && str_equals(g_files[i].name, old_name)) {
			str_copy(g_files[i].name, new_name, 64);
			return true;
		}
	}
	return false;
