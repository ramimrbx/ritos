...
extern "C" {
	...
	void ritos_launch_gui() {
		...
		void* entry = rit::System::load_rbx("/system/shell/console_shell.rbx");
		if (entry != nullptr) {
			typedef void (*shell_entry_t)(const RitOS_API* api);
			((shell_entry_t)entry)(rit::System::get_api_table());
		} else {
			rit::System::clear_screen();
			rit::System::println("CRITICAL ERROR: Failed to load /system/shell/console_shell.rbx!");
			while (1) { __asm__ volatile("cli; hlt"); }
		}
#else
		sdbg("launch_gui: load desktop.rbx\r\n");
		void* entry = rit::System::load_rbx("/system/shell/desktop.rbx");
		...
		if (entry != nullptr) {
			...
			rit::System::clear_screen();
			rit::System::println("CRITICAL ERROR: Failed to load /system/shell/desktop.rbx!");
			while (1) { __asm__ volatile("cli; hlt"); }
		}
#endif
	}
